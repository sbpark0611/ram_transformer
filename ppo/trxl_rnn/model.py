...
class TransfoXL(nn.Module):
    def __init__(self, config):
        ...
        self.word_emb = nn.Embedding(config.num_labels + 1, config.d_embed // 2)
        self.grid_emb = nn.Sequential(
            nn.Linear(config.d_rnn, config.d_embed - 1), nn.ReLU(inplace=True)
        )
    ...
    def forward(
        ...
        mask_last_obs: Optional[bool] = False,
        goal_seq: Optional[torch.LongTensor] = None,
        rew_seq: Optional[torch.FloatTensor] = None,
        ...
        if mask_last_obs:
            input_ids = torch.cat(
                [input_ids, torch.ones_like(input_ids[:, :1]) * self.n_token], dim=1
            )

        if goal_seq is None:
            goal_seq = torch.ones_like(input_ids) * self.n_token

        bsz, qlen = input_ids.size()
        obs_emb = self.word_emb(input_ids)
        goal_emb = self.word_emb(goal_seq)
        ...
        rew_emb = rew_seq.transpose(0, 1).contiguous()[..., None]
        word_emb = torch.cat([_grid_seq, obs_emb, goal_emb, rew_emb], dim=-1)
...
class xlTEM(nn.Module):
    ...
    def forward(
        ...
        g = []
        bs, d_rnn = prev_hidden.shape
        seq_len = act.shape[1]
        if not mask_last_obs:
            if torch.all(act[:, 0] == 5):
            ...
            for n in range(seq_len):
                if rew is not None:
                    if torch.all(rew[:, n] == 0):
                        # Episode start no act!
                        prev_hidden = self.rnn(act[:, n], prev_hidden)
                    else:
                        prev_hidden = (1.0 - rew[:, n : n + 1]) * prev_hidden + rew[
                            :, n : n + 1
                        ] * self.rnn.init_hidden(act[:, n])
                else:
                    prev_hidden = self.rnn(act[:, n], prev_hidden)
                g.append(prev_hidden)
