...
class HamiltonianPathGenerator:
    ...
    def findHamiltonianPaths(self, graph, n, num_paths_per_nodes=None):
        ...
        return reduce(
            lambda x, y: x + y, self._paths
        )  # concat list of per node path list into single list
...
def create_2d_graph(m, n, add_diag_edges=True, draw=False):
    ...
    if add_diag_edges:
        ...
        G.add_edges_from(
            [((x, y), (x + 1, y + 1)) for x in range(m - 1) for y in range(n - 1)]
            + [((x + 1, y), (x, y + 1)) for x in range(m - 1) for y in range(n - 1)],
            weight=1.414,
        )
...
def create_2d_connected_sequences(m, n, diag_neighbors=True, seed=0, num_paths=None):
    ...
    Gi = nx.convert_node_labels_to_integers(
