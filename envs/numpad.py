...
from envs.utils import create_2d_connected_sequences
...
class NumPadEnv(gym.Env):
    ...
    def __init__(
        ...
        set_cues: Optional[np.ndarray] = None,
        seed: int = 0,
    ):
        ...
        if custom_maps is not None:
            ...
            if neighbor_sequence:
                self.maps = self.create_hamiltonian_numpads(
                    n_maps=n_maps,
                    shape=(size, size),
                    cues=cues,
                ...
                self.maps = self.create_numpads(
                    n_maps=n_maps, shape=(size, size), cues=cues, seed=seed
                )
        ...
        if set_cues is not None:
            for i in range(self.n_maps):
                self.maps[i][1] = set_cues[i].reshape(self.tile_size, self.tile_size)
    ...
    def reset(self):
        ...
        possible_rwd_cycles = (self.steps_per_episode - 1) / self.rwd_seq_1cycle_dist(
            self.map[3]
        )
        ...
        obs = np.array([cue, 0, 0], dtype=np.int32)
        invalid_actions = []
    ...
    def create_hamiltonian_numpads(
        ...
        all_seqs = create_2d_connected_sequences(
            I, J, diag_neighbors=diag_neighbors, seed=seed, num_paths=n_maps
        )
        for i in range(n_maps):
            cues = rng.choice(cues, size=(n_cues,), replace=len(cues) < n_cues)
            rwds = [(2 * i, 2 * j) for i in range(I) for j in range(J)]
...
if __name__ == "__main__":
    ...
    env = make_vec_env(
        NumPad3x3,
        n_envs=n_envs,
        env_kwargs=dict(n_maps=3, diag_neighbors=False, cues=range(10)),
    )
