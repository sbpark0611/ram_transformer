...
class MemoryPlanningGame(gym.Env):
    ...
    def step(self, action):
        ...
        # Reverse mode changes
        # if self._episode_steps >= self._max_episode_steps // 2 and self._reverse_mode in [1] and not self.switched:
        #     self._goal = self._set_opposite_position(self._goal)
        #     self.switched = True
    ...
    def _set_opposite_position(self, pos):
        n = self._maze_size
        x, y = pos
        # Set position to the opposite site considering periodic boundary condition
        x, y = (x + n//2) % n, (y + n//2) % n
        return (x, y)
